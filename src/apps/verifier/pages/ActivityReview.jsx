...
export default function ActivityReview() {
  ...
  const reviewedActivities = activities.filter((activity) => activity.status === 'approved' || activity.status === 'rejected');
  const approvedCount = reviewedActivities.filter((activity) => activity.status === 'approved').length;
  const rejectedCount = reviewedActivities.filter((activity) => activity.status === 'rejected').length;
    ...
    <>
      ...
      <section>
        <div className="card mb-6" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', padding: '1.25rem 1.75rem' }}>
          <div>
            <h3 style={{ marginBottom: '0.25rem' }}>Activity Review</h3>
            <p className="text-muted" style={{ margin: 0 }}>Approved and rejected cleanup activities.</p>
          </div>
        ...
        {reviewedActivities.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
            <p className="text-muted">No approved or rejected activities yet.</p>
          </div>
        ) : (
