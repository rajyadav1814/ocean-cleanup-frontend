        ...
        <div className="content-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
          {activities.map((activity) => (
            <div key={activity.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {/* Image Display */}
              <div style={{ background: '#1c1c1c', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#555', overflow: 'hidden' }}>
                {activity.imageUrl ? (
                  <img src={activity.imageUrl} alt="Cleanup evidence" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
              ...
              <div style={{ padding: '1.5rem' }}>
                <div className="flex-between mb-4">
                  <h4 style={{ margin: 0, color: '#fff', fontSize: '1.1rem' }}>{activity.location || 'Zuma Beach cleanup'}</h4>
                  <span style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                  <div className="flex-between">
                    ...
                    <strong style={{ color: '#fff', fontWeight: '500' }}>{activity.organization || 'Ocean Guardians'}</strong>
                  </div>
                  ...
                  <div className="flex-between">
                    ...
                    <strong style={{ color: '#fff', fontWeight: '500' }}>{activity.gps || '34.0259, -118.8231'}</strong>
                  </div>
                  ...
                  <div className="flex-between">
                    ...
                    <strong style={{ color: '#fff', fontWeight: '500' }}>{activity.volunteers || '23'}</strong>
                  </div>
                  ...
                  <div className="flex-between">
                    ...
                    <strong style={{ color: '#fff', fontWeight: '500' }}>{activity.quantity || '47'} kg</strong>
                  </div>
                </div>

                {/* Action Buttons */}
                <div className="mt-6 flex-between gap-4">
                  <button style={{ flex: 1, background: '#16a34a', color: '#000', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', boxShadow: 'none' }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    Approve
                  </button>
                  <button style={{ flex: 1, background: 'transparent', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', boxShadow: 'none' }}>
